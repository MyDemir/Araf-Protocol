...
/**
 * ArafEscrow — Full Test Suite (v2.1 compatible)
 *
 * Critical fixes from original test file:
 *  - K-04: mockUSDT.mint(addr, amount) now works → MockERC20 has admin mint(address,uint256)
 *  - K-05: AmountExceedsTierLimit tests added (Tier 0–4 caps enforced on-chain)
 *  - BUG:  pingMaker requires GRACE_PERIOD (48h) after paidAt, NOT 24h
 *  - BUG:  autoRelease requires pingMaker first (MustPingFirst guard)
 *  - BUG:  setupTrade(0) with TRADE_AMOUNT=1000 USDT exceeds Tier 0 limit (150 USDT)
 *  - FIX:  TIER0/1 cooldown is 4h (not 24h) → labels and time.increase corrected
 *  - FIX:  Bond amounts recalculated with good-rep discount:
 *          Tier 2 maker: 600-100=500 BPS=5% → 50 USDT (not 60)
 *          Tier 2 taker: 800-100=700 BPS=7% → 70 USDT (not 80)
 */
...
describe("ArafEscrow", function () {
  ...
  async function deployAndSetupFixture() {
    ...
    // K-04 FIX: uses mint(address, uint256) — now available on MockERC20
