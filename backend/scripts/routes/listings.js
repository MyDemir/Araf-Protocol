...
// ─── Bond oranları — ArafEscrow.sol sabitleriyle senkronize ──────────────────
// Kontrat: T0:%0/%0, T1:%8/%10, T2:%6/%8, T3:%5/%5, T4:%2/%2
// Tier 0: Yeni kullanıcı teşviki — bond yok, sadece kilitli crypto erimeye tabidir.
const BOND_MAP = {
  0: { maker: 0,  taker: 0  },
  1: { maker: 8,  taker: 10 },
  2: { maker: 6,  taker: 8  },
...
router.post("/", requireAuth, listingsWriteLimiter, async (req, res, next) => {
  try {
    ...
    const bonds = BOND_MAP[value.tier];
