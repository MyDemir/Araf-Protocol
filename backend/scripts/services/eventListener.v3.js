...
class EventWorker {
  ...
  async _upsertChildTradeFromChain(tradeId, extraSet = {}) {
    ...
    if (onchainTrade.parentOrderId > 0) {
      orderSnapshot = await this._syncOrderMirrorFromChain(onchainTrade.parentOrderId);
    }

    const orderSide = orderSnapshot ? this._normalizeOrderSide(orderSnapshot.side) : null;
    ...
    await this._upsertTradeMirrorRaw(
      { onchain_escrow_id: tradeIdNum },
      {
        $set: {
          ...
          order_side_snapshot: orderSide?.label || null,
          order_side_snapshot_code: orderSide?.code ?? null,
          listing_id: null,
          financials: {
            crypto_amount: onchainTrade.cryptoAmount,
            crypto_amount_num: this._safeNumber(onchainTrade.cryptoAmount, 0),
            total_decayed: "0",
            total_decayed_num: 0,
            decay_tx_hashes: [],
            decayed_amounts: [],
          ...
          maker_bond: onchainTrade.makerBond,
          maker_bond_num: this._safeNumber(onchainTrade.makerBond, 0),
          taker_bond: onchainTrade.takerBond,
          taker_bond_num: this._safeNumber(onchainTrade.takerBond, 0),
          taker_fee_bps_snapshot: onchainTrade.takerFeeBpsSnapshot,
          maker_fee_bps_snapshot: onchainTrade.makerFeeBpsSnapshot,
          child_listing_ref: extraSet.child_listing_ref || null,
          updated_at: new Date(),
          ...extraSet,
        },
        $setOnInsert: {
    ...
    return { onchainTrade, orderSnapshot };
  }
  ...
  async _onOrderFilled(event) {
    ...
    await this._syncOrderMirrorFromChain(orderIdNum, {
      last_filled_trade_id: tradeIdNum,
      last_filler_address: this._normalizeAddress(filler),
      last_fill_amount: this._safeString(fillAmount, "0"),
      last_fill_amount_num: this._safeNumber(fillAmount, 0),
    ...
    await this._upsertChildTradeFromChain(tradeIdNum, {
      ...
      last_filler_address: this._normalizeAddress(filler),
      fill_amount: this._safeString(fillAmount, "0"),
      fill_amount_num: this._safeNumber(fillAmount, 0),
      child_trade_spawned_from: "ORDER_FILLED",
    });
  }
  ...
  async _onOrderCanceled(event) {
    ...
    await this._syncOrderMirrorFromChain(orderIdNum, {
      ...
      remaining_amount: this._safeString(remainingAmount, "0"),
      remaining_amount_num: this._safeNumber(remainingAmount, 0),
      last_maker_bond_refund: this._safeString(makerBondRefund, "0"),
      last_maker_bond_refund_num: this._safeNumber(makerBondRefund, 0),
      last_taker_bond_refund: this._safeString(takerBondRefund, "0"),
      last_taker_bond_refund_num: this._safeNumber(takerBondRefund, 0),
  ...
  async _onEscrowCreated(event) {
    ...
    if (onchainTrade.parentOrderId > 0) {
      const { orderSnapshot } = await this._upsertChildTradeFromChain(tradeIdNum, {
        child_listing_ref: listingRef,
        created_via_v3_order_flow: true,
      });

      if (orderSnapshot) {
        await this._syncOrderMirrorFromChain(onchainTrade.parentOrderId, {
          last_child_trade_id: tradeIdNum,
          last_child_listing_ref: listingRef,
          updated_at: new Date(),
        });
      }
    ...
    if (isZeroRef) {
      ...
      throw new Error("CRITICAL: zero listingRef — kontrat fix öncesi oluştu");
    }
    ...
    if (!listing) {
      ...
      throw new Error(`listing_ref bulunamadı: ${listingRef}`);
    }
    ...
    if (!makerMatches || !tierMatches || !tokenMatches) {
      ...
      throw new Error("listing_ref bulundu fakat on-chain maker/tier/token doğrulaması başarısız.");
    }

    if (listing.onchain_escrow_id !== null && listing.onchain_escrow_id !== tradeIdNum) {
      throw new Error(`listing_ref başka escrow'a bağlı: ${listing.onchain_escrow_id}`);
    }
    ...
    if (listing.onchain_escrow_id === null) {
      ...
      if (!linkResult.modifiedCount) {
        ...
        throw new Error("Authoritative listing link race — onchain_escrow_id atomik bağlanamadı.");
      }
    }
    ...
    const financials = {
      crypto_amount: this._safeString(amount, "0"),
  ...
  async _onEscrowLocked(event) {
    const { tradeId, taker, takerBond } = event.args;
    const tradeIdNum = this._safeNumber(tradeId);
    ...
    await this._upsertTradeMirrorRaw(
      { onchain_escrow_id: tradeIdNum },
      {
        $set: {
          ...
          taker_bond: this._safeString(takerBond, "0"),
          taker_bond_num: this._safeNumber(takerBond, 0),
          updated_at: new Date(),
