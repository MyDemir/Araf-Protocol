...
* **Tier and Asymmetric Collateral System:** A 5-tier reputation system protects experienced users from new and untested actors. Higher Tiers mean lower collateral rates and higher transaction limits. **Limits are strictly enforced in absolute crypto amounts (USDT/USDC) to prevent fiat rate manipulation.**
...
* **Encrypted Receipt Storage & Right to be Forgotten:** Receipts are AES-256-GCM encrypted off-chain and automatically permanently deleted after trade resolution or a set TTL (Time-to-Live).
* **Triangulation Fraud Prevention:** The Maker is explicitly shown the Taker's decrypted real name in the Trade Room to verify the sender's identity, preventing third-party scams.
...
*   **Tier ve Asimetrik Teminat Sistemi:** 5 kademeli itibar sistemi, tecrübeli kullanıcıları yeni ve test edilmemiş aktörlerden korur. Yükselen Tier'lar, daha düşük teminat oranları ve daha yüksek işlem limitleri anlamına gelir. **Kur manipülasyonunu (rate manipulation) engellemek adına limitler tamamen kripto varlık (USDT/USDC) bazında katı şekilde uygulanır.**
...
*   **Şifreli Dekont Depolama ve Unutulma Hakkı:** İşlem dekontları off-chain olarak AES-256-GCM ile şifrelenir ve işlem tamamlandıktan sonra (veya belirli bir TTL süresi sonunda) kalıcı olarak silinir.
*   **Üçgen Dolandırıcılık (Triangulation Fraud) Koruması:** Satıcıya (Maker), gelen paranın kaynağını teyit edebilmesi için İşlem Odasında alıcının (Taker) şifresi çözülmüş gerçek adı gösterilir; böylece üçüncü şahıs dolandırıcılıklarının önüne geçilir.
